...
async def on_message(message):
  ...
  if message.author.guild_permissions.administrator == True:
    ...
    if message.content.startswith('!enable'):
      await message.channel.send("You're next Funky Monkey Friday alert is scheduled for {}:{} on the next Friday".format(h,m))
      @tasks.loop(seconds=1.0)
      async def alerts():
        print(datetime.datetime.now(tz=pytz.timezone(ZoneSelection)))
        # weekday() == 4 would be Friday
        if datetime.datetime.now(tz=pytz.timezone(ZoneSelection)).today().weekday() == 4 and datetime.datetime.now(tz=pytz.timezone(ZoneSelection)).hour == h and datetime.datetime.now(tz=pytz.timezone(ZoneSelection)).minute ==  m:
          await message.channel.send("@everyone **IT'S FUNKY MONKEY FRIDAY! SEIZE THE DAY!**", file=discord.File("FunkyMonkeyGifs/" + RandomGif))
          time.sleep(60)
        print('alerts are active for {}:{}'.format(h,m))
      alerts.start()
